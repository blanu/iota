...
class Storage:
    ...
    def dispatchMonad(self, f):
        ...
        elif f.t == StorageType.INTEGER_ARRAY:
...
class Integer(Storage):
    ...
    def __init__(self, x, o=ObjectType.INTEGER):
        super().__init__(o, StorageType.INTEGER, int(x))
    ...
    def dyad(self, x, rop, op):
        ...
        elif x.t == StorageType.INTEGER_ARRAY:
    ...
    def rotate(self, x):
        if x.t == StorageType.INTEGER_ARRAY:
...
class Float(Storage):
    ...
    def __init__(self, x, o=ObjectType.REAL):
...
class IntegerArray(Storage):
    ...
    def __init__(self, x, o=ObjectType.LIST):
        super().__init__(o, StorageType.INTEGER_ARRAY, [int(y) for y in x])
            ...
            if len(x.i) == 0:
                ...
                for y in x.i:
                    if y >= 0.0:
                        ...
                        if previous is None:
                            results.append(IntegerArray(self.i[:last]))
                            previous = last
                        else:
                            if previous <= last:
                                results.append(IntegerArray(self.i[previous:last]))
                                previous = last
                            else:
                                return Error('invalid argument value')
                    else:
                ...
                if previous is None:
                    ...
                    results.append(IntegerArray(self.i[previous:]))
                    return MixedArray(results)
        elif x.t == StorageType.MIXED_ARRAY:
            if len(x.i) == 0:
                return self
                ...
                first = x.i[0]
                if first.t != StorageType.INTEGER:
                    return Error('invalid argument type')
                rest = x.i[1:]
                results = []
                for y in rest:
                    last = y
                    if last.t != StorageType.INTEGER:
                        return Error('invalid argument type')
                    if first.i <= last.i:
        ...
        if x.t == StorageType.INTEGER:
            ...
            if len(x.i) == 0:
                ...
                for y in x.i:
                    if y >= 1:
                        if previous is None:
                            results.append(IntegerArray(self.i[:y-1]))
                            previous = y
                        else:
                            if previous <= y:
                                results.append(IntegerArray(self.i[previous-1:y-1]))
                                previous = y
                            else:
                                return Error('invalid argument value')
                    else:
                ...
                if previous is None:
                    ...
                    results.append(IntegerArray(self.i[previous-1:]))
                    return MixedArray(results)
        elif x.t == StorageType.FLOAT_ARRAY:
            return Error('invalid argument type')
        elif x.t == StorageType.MIXED_ARRAY:
...
class MixedArray(Storage):
    ...
    def transpose(self):
        if len(self.i) == 0:
            return self
        elif all(map(lambda y: y.t == StorageType.INTEGER_ARRAY, self.i)):
    ...
    def shape(self):
        if len(self.i) == 0:
            ...
            firstShape = shapes[0]
            if firstShape.t == StorageType.INTEGER: # atom in array means shape is simple vector type
                return IntegerArray([len(self.i)])
            elif firstShape.t == StorageType.INTEGER_ARRAY:
...
class Error(Storage):
    def __init__(self, x):
        super().__init__(ObjectType.ERROR, StorageType.ERROR, x)
...
class Dictionary:
    def __init__(self, keys, values):
        self.map = {}
        if keys.t == StorageType.INTEGER_ARRAY:
    ...
    def put(self, key, value):
        ...
        for key, value in newMap.items():
            keys.append(key)
            values.append(value)
        if all(map(lambda y: y.t == StorageType.INTEGER, keys)):
            if all(map(lambda y: y.t == StorageType.INTEGER, values)):
    ...
    def keys(self):
        ...
        if all(map(lambda y: y.t == StorageType.INTEGER, result)):
    ...
    def items(self):
        results = []
        for key, value in self.map.items():
            if key.t == StorageType.INTEGER and value.t == StorageType.INTEGER:
